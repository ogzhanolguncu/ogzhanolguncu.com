...
After learning map, reduce and higher order functions, you finally stepped into the gate of functional programming.
As you keep delving deeper you stumbled upon composing and piping, and, you start to wonder why one even uses compose.
Function composition(compose) allows us to define reusable, testable and maintainable functions, actually all those perks are motivation behind functional programming existance.
This whole process quite paralel to what we do in math.
...
Just like in this example, compose function uses output of the functions as input for next function. Let's see it in action.
...
We can even seperate arrow functions to their own functions to make it more readable.
...
given array which in our case it's our curried value - `val` -. Now, instead of chaining filter and map let's compose them.
...
By the way, if reading right to left feels wierd don't worry I got you covered. There is another function called `pipe`, works just like `compose` but in reverse.
All we have to do use `reduce` instead of `reduceRight`.
...
We can even spice it up our compose/pipe more with partial application, by the way partial application is just like currying allows you to make multiple function calls, but
also gives you an option provide multiple arguments. In our case we will provide only one argument for the sake of simplicity. Let's see it in the action.
...
Let's imagine we have a person object, and we been requested to make few changes without mutating the object.
Because, object are passed around by their references which results in mutating the object and losing the previous version of object which generates impure functions.
In functional programming we always strive for pure functions. To overcome that hurdle, we need to make shallow copy of our object first, then mutate the copied object.
...
It's been quite an adventure, we learned lots of new things, things that takes time to digest. Such as currying to call function multiple times with different arguments,
...
concepts gives us side-effect free(pure functions), immutable objects, testable and reusable functions. Examples we went through were quite trivial, but it can be used on complicated scenarios as well.
This is just beginning of functional programming if you are into it definitely check monads, transducers, functors.
