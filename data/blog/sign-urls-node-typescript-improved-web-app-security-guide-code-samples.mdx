...
1.  [Extracting data out of URL](#extracting-data-out-of-url)
1.  [Signing the URL](#signing-the-url)
...
Finally, we inspect the expiration date and verify that it is set for a future date. If not, the process has failed. If all conditions are met, we return `true`. Always keep your URLs safe.
