...
![react-testing-library](/static/images/selection-sort-in-javascript/selection.png)
...
Imagine a scenario where you develop a new component that works with your old component, you might be somehow modified your all component, so how do you make sure your old component is working as you expected?.
...
`mockedAxios.mockImplementationOnce(() => Promise.resolve(singularJoke));` => We call method called `mockImplementationOnce()` to mock a request, and tell it to return a single joke.
...
`mockedAxios.mockImplementationOnce(() => Promise.resolve(emptySingularStory));` => We want to test empty joke case.
