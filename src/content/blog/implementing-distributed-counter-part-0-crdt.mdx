...
Let's discuss the whats and whys of **CoW**(Copy-on-Write)[<a href="#ref7">7</a>], **CAS**(Compare and Swap)[<a href="#ref6">6</a>], Object Pooling and Ref Counting. I know we've discussed quite a bit of theory but, bear with me, we just need a bit more.
...
**Here's the breakdown of the Increment:**

First, we get a pointer to the current increments map. Then, we grab a temporary map from our map pool. We copy the data from the current map into our temporary one, leaving the original untouched for now. We apply the increment operation on this temporary map.

If the CAS succeeds, we release our hold on the old map (which is essential for our reference counting mechanism, allowing it to be garbage collected later) and the increment is successful. The beauty here is that if the CAS fails (likely because another concurrent operation completed first), nothing has been corrupted; the original map is still intact, and we simply return our unused temporary map to the pool before the operation retries.
...
This is almost identical to `Increment` method with minor differences. Instead of incrementing the increments map, we increment the decrements map. The rest of the operations are the same.
...
Here's the breakdown of `MergeIncrements`, which shares similarities with `Increment`/`Decrement` as it's also a write operation:

Like before, we start by getting a pointer to the current `increments` map. However, since we're merging data from another counter (`other PNMap`), we have an opportunity for optimization _before_ doing any copying or swapping. We check if the incoming map actually contains any information that would change our current state (i.e., if it has counts for nodes we don't know, or higher counts for nodes we do know). If no update is needed based on this check, we can return early, saving unnecessary work – a nice optimization.

If an update _is_ required, we proceed with CoW. Now, the core merge logic specific to CRDT PN-Counters happens: we iterate through the incoming `other` map and update our temporary map by taking the **maximum** value seen for each node ID. This ensures that we preserve the highest count observed across both maps, which is key to convergence.

Here’s a simple illustration of the "take the max" rule for three nodes:

```sh
   Merge Logic Example (Taking Max):

   Local Map (Current State)      Incoming Map (Other State)      Resulting Merged Map (In Temp Map)
   +---------+                    +---------+                     +------------------+
...
Finally, we attempt CAS to make our merged temporary map the new current `increments` map. Just like with `Increment`:

- **If the CAS succeeds:** We release our hold on the old map and return `true`
- **If the CAS fails:** Another concurrent operation likely finished first. We return our unused temporary map to the pool and signal that the operation needs to be retried
...
Same logic but this time for decrements map.
...
func (p *PNCounter) Merge(other *PNCounter) bool {
	...
	// [Step 2] Merge the increment maps using the dedicated merge method.
	incUpdated := p.MergeIncrements(otherIncrements)

	// [Step 3] Merge the decrement maps using the dedicated merge method.
