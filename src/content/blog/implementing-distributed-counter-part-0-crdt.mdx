...
The `Increment` method acquires an exclusive lock `(incMu.Lock())` to safely update the `increments` map for the given `nodeId`, initializing it if necessary before incrementing. `Decrement` follows the identical pattern for the `decrements` map using `decMu`.
...
This helper function safely retrieves copies of the internal maps, useful when sending state to another node or preparing for a merge.
...
This ensures read operations on the source counter are separated from write operations on the destination counter during the merge.
...
func TestPNCounter_ConcurrentMerges(t *testing.T) {
	...
	for _, src := range sources {
		go func(source *PNCounter) {
			...
			for i := 0; i < 3; i++ {
				main.Merge(source) // Use the Merge method
			}
		}(src)
	}
